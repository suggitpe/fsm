...
/**
 * The internal interface for a state machine, offering additional
 * construction and optimisation methods over the public state machine
 * interface
 */
public interface IScribeStateMachine extends IStateMachine {

    /**
     * Sets the BehavioredClassifier to be used by the state machine.
     * The behaviored classifier contains the state machine, which is
     * a kind of behavior.
     */
    void setBehavioredClassifier(IBehavioredClassifier context);

    /**
     * Sets the INamespaceObject manager to be used by the state
     * machine for storing and retrieving references to namespace
     * objects.
     */
    void setNamespaceObjectManager(INamespaceObjectManager namespaceObjectManager);

    /**
     * Populates the INamespaceObjectManager with name -> object
     * references pairs by inspecting the state machine object graph.
     */
    void initialiseNamespaceContext();

    /**
     * Checks the internal state machine using the specified
     * IConstraintVisitor instance.
     */
    ...
    /**
     * Sets the IEventContextFactory to use when creating
     * IEventContext objects for events.
     */
    ...
    /**
     * Sets the event factory to be used by the state machine to
