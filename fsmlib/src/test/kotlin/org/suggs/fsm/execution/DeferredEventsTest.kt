...
class DeferredEventsTest {
    ...
    @Test
    fun `fires deferred events automatically`() {
        val executionEnvironment = createAStateMachineContextWithDeferredEvents()
        sendEventsToDeferIn(executionEnvironment)
        stateManager.deferredEvents.size shouldBeExactly 2

        executionEnvironment.handleEvent(aBusinessEventCalled("realEvent"))

        stateManager.printAudits()
        theResultingState() shouldEndWith DEFAULT_FINAL_STATE_NAME
    }

    @Test
    fun `transitions automatic transitions over deferred events`() {
        ...
        stateManager.printAudits()
        theResultingState() shouldEndWith "state3"
    }
